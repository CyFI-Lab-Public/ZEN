...
class Python_3_8_18_IntermedSymbols(intermed.IntermediateSymbolTable):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.set_type_class("PyFunctionObject", PyFunctionObject)    # https://github.com/python/cpython/blob/v3.7.13/Include/funcobject.h#L41
        self.set_type_class("PyCodeObject", PyCodeObject)            # https://github.com/python/cpython/blob/v3.8.18/Include/code.h#L69
        ...
        self.set_type_class("PyModuleObject", PyModuleObject)        # https://github.com/python/cpython/blob/main/Include/moduleobject.h#L93
...
class PyTypeObject(objects.StructType):
    ...
    def get_bases(self):
        tp_bases = self.tp_bases.dereference().get_value()
        base_list = []
        for base in tp_bases:
            try:
                base_list.append(base.get_name())
            except:
                print(f" Inspect this base!!")
                continue
        return base_list
...
class PyFunctionObject(objects.StructType):    
    def get_name(self):
        """
        Gets the value of the __name__ attribute of this function.
        """
        curr_layer = self._context.layers[self.vol.layer_name]
        if self.name != 0:
            name_obj = self.name.dereference()
        else:
            print("empty function name field")
            return 'empty_name'
        return name_obj.get_value()
    
    def get_module(self):
        """
        Gets the value of the __module__ attribute of this function.
        """
        if self.func_module != 0:
            md_obj = self.func_module.dereference()
        else:
            return 'No module field'
        return md_obj.get_value()
    
    def get_qualname(self):
        """
        Gets the value of the qualname attribute of this function.
        """
        if self.qualname != 0:
            qualname = self.qualname.dereference()
        else:
            return 'NO qualname field'
        return qualname.get_value()


class PyCodeObject(objects.StructType):
    def get_value(self):
        """
        Gets the native Python code object, ready for marshalling.

        """
        co_filename = self.co_filename.dereference().get_value()
        co_argcount = self.co_argcount
        co_posonlyargcount = self.co_posonlyargcount
        co_kwonlyargcount = self.co_kwonlyargcount
        co_nlocals = self.co_nlocals
        co_stacksize = self.co_stacksize
        co_flags = self.co_flags
        co_firstlineno = self.co_firstlineno
        co_code = self.co_code.dereference().get_value()
        co_consts = self.co_consts.dereference().get_value()
        co_names = self.co_names.dereference().get_value()
        co_varnames = self.co_varnames.dereference().get_value()
        co_freevars = self.co_freevars.dereference().get_value()
        co_cellvars = self.co_cellvars.dereference().get_value()
        co_name = self.co_name.dereference().get_value()
        co_linetable = self.co_linetable.dereference().get_value()
...
class PyModuleObject(objects.StructType):
    def get_value(self):
        return
    def get_type(self):
        return 'PyModuleObject'
